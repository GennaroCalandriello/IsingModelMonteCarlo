...
monte_history_m = (
    False  # grafico passo montecarlo per la magnetizzazione a fissati beta
)
...
if monte_history_m == True:
    ...
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        magnetization = np.loadtxt(
            f"results/lattice_dim_{i}/magnetization/magnetization_beta_{beta_num:.3f}.txt"
        )
...
if monte_history_e == True:
    ...
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        energy = np.loadtxt(
            f"results/lattice_dim_{i}/energies/energies_beta_{beta_num:.3f}.txt"
        )
...
if mean == True:
    ...
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        ...
        mean = np.loadtxt(
            f"results_analysis/magnetizzazione_media/magnetizzazione_media_lattice_dim_{i}.txt"
        )
        error = np.loadtxt(
            f"results_analysis/magnetizzazione_media/sigma_mean_magn_lattice_dim_{i}.txt"
        )
        plt.errorbar(
            bet,
            mean,
            yerr=error * 2,
            color=color_palette[round(i / 10) - 1],
            label=f"Lattice: {i}",
            fmt=".",
            mfc="red",
            mec="green",
...
if chi == True:
    ...
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        ...
        err_susc = np.loadtxt(
            f"results_analysis/suscettività/sigma_susceptibility_lattice_dim_{i}.txt"
        )
        plt.errorbar(
            bet,
            susc,
...
if heat == True:
    ...
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        sh = np.loadtxt(
            f"results_analysis/calore_specifico/calore_specifico_lattice_dim_{i}.txt"
        )
        err_sh = np.loadtxt(
            f"results_analysis/calore_specifico/sigma_specific_heat_lattice_dim_{i}.txt"
        )
        beta_resc = np.loadtxt(
...
if mean_r == True:
    plt.figure()
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        magn_resc = np.loadtxt(
            f"results_analysis/magnetizzazione_riscalata/magnetizzazione_riscalata_lattice_dim_{i}.txt"
        )
        err_magn = np.loadtxt(
            f"results_analysis/magnetizzazione_media/sigma_mean_magn_lattice_dim_{i}.txt"
        )
...
if binder == True:
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        ...
        err_bind = np.loadtxt(
            f"results_analysis/binder/binder_sigma_lattice_dim_{i}.txt"
        )
        plt.errorbar(
            beta_range,
            binder,
...
if susc_r == True:
    for i in range(latt_dim_start, latt_dim_stop, passo_latt_dim):
        susc_resc = np.loadtxt(
            f"results_analysis/suscettività_riscalata/suscettività_riscalata_lattice_dim_{i}.txt"
        )
        err_susc = np.loadtxt(
            f"results_analysis/suscettività/sigma_susceptibility_lattice_dim_{i}.txt"
        )
...
if histogram == True:
    magn_0_35 = np.loadtxt(
        "results/lattice_dim_50/magnetization/magnetization_beta_0.350.txt"
    )
    magn_0_45 = np.loadtxt(
        "results/lattice_dim_50/magnetization/magnetization_beta_0.450.txt"
    )
    plt.figure()
